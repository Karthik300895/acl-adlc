...
4. **Manager review gate.** Set `brief.md` frontmatter `status: pending-review` and `updated: {date}` (not `final`). Tell the user: push to GitHub; a manager Approves or Rejects in `acl-docs-console.html` — that only changes the YAML `status` line in the MD file; pull before any downstream ACL skill can proceed.
5. Tell the user it is ready: local paths and external destinations (URLs returned from handoffs). Invoke `acl-help` to suggest what next steps make sense in the acl method ecosystem.
