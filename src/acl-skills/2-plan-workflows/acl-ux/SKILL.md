...
- **Polished, handed off, closed.** Apply `{workflow.doc_standards}` in order. Execute `{workflow.external_handoffs}`; surface URLs. Set both files' `status: pending-review`, `updated: {date}` (manager Approve/Reject via `acl-docs-console.html` — updates only the YAML `status` line; downstream skills require `approved`). Log finalization via `uv run {project-root}/_acl/scripts/memlog.py append --workspace {doc_workspace} --type event --text "spines pending-review"`. Share paths. Common next after approval: `acl-architecture`, `acl-create-epics-and-stories`, `acl-quick-dev`. Run `{workflow.on_complete}`.
