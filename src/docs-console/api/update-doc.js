...
function repoConfig() {
  ...
  if ((!owner || !repo) && process.env.GITHUB_REPO_URL) {
    const match = process.env.GITHUB_REPO_URL.match(
      /github\.com[/:]([^/]+)\/([^/.]+)(?:\.git)?/i,
    )
    if (match) {
      owner = owner || match[1]
  ...
  try {
    ...
    if (editSecret) {
      const provided =
        ...
        req.headers['x-docs-edit-secret'] ||
        ''
    ...
    const repoProbe = await githubFetch(
    ...
    if (repoProbe.res.status === 404) {
      json(res, 403, {
        error:
          `GitHub token cannot access ${owner}/${repo}. Edit the fine-grained PAT and grant this repository with Contents: Read and write, then update GITHUB_TOKEN in Vercel and redeploy.`,
      })
      return
    }
    if (!repoProbe.res.ok) {
    ...
    json(res, 200, {
      ...
      message:
        'Saved to GitHub. This Vercel deployment still serves the previous build until the next deploy.',
    })
  } catch (err) {
    json(res, 500, {
