...
const DEFAULT_NVIDIA_MODEL = process.env.LLM_MODEL || 'meta/llama-3.2-11b-vision-instruct';

function resolveLlmApiKey(requestApiKey) {
  return process.env.LLM_API_KEY || requestApiKey || '';
}
...
`;
  ...
  try {
    const resolvedApiKey = resolveLlmApiKey(apiKey);
    if (!resolvedApiKey) {
      throw new Error('LLM_API_KEY is not configured on the studio server.');
    }
