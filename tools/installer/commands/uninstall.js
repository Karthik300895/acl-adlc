...
module.exports = {
  command: 'uninstall',
  description: 'Remove ACL installation from the current project',
  options: [
    ['-y, --yes', 'Remove all ACL components without prompting (preserves user artifacts)'],
    ...
    try {
      ...
      if (options.directory) {
        ...
        const dirChoice = await prompts.select({
          message: 'Where do you want to uninstall ACL from?',
          choices: [
      ...
      if (!(await fs.pathExists(aclDir))) {
        await prompts.log.warn('No ACL installation found.');
        process.exit(0);
      }
      ...
      await prompts.intro('ACL Uninstall');
      ...
      if (!options.yes) {
        ...
        const selected = await prompts.multiselect({
          ...
          options: [
            {
              value: 'modules',
              label: `ACL Modules & data (${installer.aclFolderName}/)`,
      ...
      // Phase 3: ACL modules & data (last — other phases may need _acl/)
      if (removeModules) {
        const s = await prompts.spinner();
        s.start(`Removing ACL modules & data (${installer.aclFolderName}/)...`);
