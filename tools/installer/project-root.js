...
/**
 * Find the ACL project root directory by looking for package.json
 * or specific ACL markers
 */
function findProjectRoot(startPath = __dirname) {
  let currentPath = path.resolve(startPath);

  // Keep going up until we find package.json with acl-adlc
  while (currentPath !== path.dirname(currentPath)) {
    ...
    if (fs.existsSync(packagePath)) {
      try {
        ...
        if (pkg.name === 'acl-adlc-v1' || pkg.name === 'acl-adlc' || fs.existsSync(path.join(currentPath, 'src', 'core-skills'))) {
          return currentPath;
        }
      } catch {
        // Continue searching
 ...
 * acl is a built-in module directly under src/
...
function getModulePath(moduleName, ...segments) {
  ...
  if (moduleName === 'acl') {
    return getSourcePath('acl-skills', ...segments);
  }
...
function getExternalModuleCachePath(moduleName, ...segments) {
  const base = process.env.ACL_EXTERNAL_MODULES_CACHE || path.join(os.homedir(), '.acl', 'cache', 'external-modules');
 ...
 *
 * Built-in modules (core, acl) live under <src>. External official modules are
 * cloned into ~/.acl/cache/external-modules/<name>/ with varying internal
 * layouts (some at src/module.yaml, some at skills/module.yaml, some nested).
 * Url-source custom modules are cloned into ~/.acl/cache/custom-modules/<host>/<owner>/<repo>/
...
async function resolveInstalledModuleYaml(moduleName) {
  ...
  // Community modules are cloned to ~/.acl/cache/community-modules/<name>/
  ...
  const communityCacheRoot = path.join(os.homedir(), '.acl', 'cache', 'community-modules', moduleName);
  ...
  // Fallback: url-source custom modules cloned to ~/.acl/cache/custom-modules/.
  ...
  // and direct-mode modules, since we identify repo roots by .acl-source.json
  // (written by cloneRepo) or .claude-plugin/ rather than by marketplace.json.
  try {
    const customCacheDir = path.join(os.homedir(), '.acl', 'cache', 'custom-modules');
