...
class Manifest {
   ...
   */
  async create(aclDir, data, installedFiles = []) {
    ...
    // Get the ACL version from package.json
    const aclVersion = data.version || require(path.join(process.cwd(), 'package.json')).version;

    // Convert module list to new detailed format
    const moduleDetails = [];
    if (data.modules && Array.isArray(data.modules)) {
      for (const moduleName of data.modules) {
        // Core and ACL modules use the ACL version
        const moduleVersion = moduleName === 'core' || moduleName === 'acl' ? aclVersion : null;
        ...
        moduleDetails.push({
          ...
          installDate: now,
          lastUpdated: now,
          source: moduleName === 'core' || moduleName === 'acl' ? 'built-in' : 'unknown',
        });
      }
    }
   ...
   */
  async read(aclDir) {
    const yamlPath = path.join(aclDir, '_config', 'manifest.yaml');
    const yaml = require('yaml');
   ...
   */
  async addModule(aclDir, moduleName, options = {}) {
    let manifest = await this._readRaw(aclDir);
    if (!manifest) {
    ...
    await this._writeRaw(aclDir, manifest);
  }
   ...
   */
  async getAllModuleVersions(aclDir) {
    const manifest = await this._readRaw(aclDir);
   ...
   */
  async _writeRaw(aclDir, manifestData) {
    const yaml = require('yaml');
    const manifestPath = path.join(aclDir, '_config', 'manifest.yaml');
   ...
   */
  async getModuleVersionInfo(moduleName, aclDir, moduleSourcePath = null) {
    // Resolve source type first, then read version with the correct path context
    if (['core', 'acl'].includes(moduleName)) {
    ...
    const customSource = await customMgr.findModuleSourceByCode(moduleName, { aclDir });
   ...
   */
  async checkForUpdates(aclDir) {
    const semver = require('semver');
    const modules = await this.getAllModuleVersions(aclDir);
    const updates = [];
