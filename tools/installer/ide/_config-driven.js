...
const { getInstalledCanonicalIds, isAclOwnedEntry } = require('./shared/installed-skills');
...
// every ACL persona is configured via [agent] in its customize.toml,
...
// catches exactly the 20 description-confirmed personas across ACL, CIS,
// GDS, WDS, TEA, and correctly excludes meta-skills like
// `acl-agent-builder` (a skill-builder workflow whose canonical id
...
// in the install flow.
async function isAgentSkill(record, aclDir) {
  if (!record?.path || !aclDir) return false;
  ...
  const tomlPath = path.join(aclDir, path.dirname(relativePath), 'customize.toml');
 ...
 *
 * This class provides a standardized way to install ACL artifacts to IDEs
...
class ConfigDrivenIdeSetup {
   ...
   * Checks for acl-prefixed entries in target_dir.
  ...
  async detect(projectDir) {
    ...
    const aclDir = await this._findAclDir(root);
    const canonicalIds = await getInstalledCanonicalIds(aclDir);
    return entries.some((e) => isAclOwnedEntry(e, canonicalIds));
  }
   ...
   */
  async setup(projectDir, aclDir, options = {}) {
    // Check for ACL files in ancestor directories that would cause duplicates
    if (this.installerConfig?.ancestor_conflict_check) {
      const conflict = await this.findAncestorConflict(projectDir);
      if (conflict) {
        await prompts.log.error(
          `Found existing ACL skills in ancestor installation: ${conflict}\n` +
            `  ${this.name} inherits skills from parent directories, so this would cause duplicates.\n` +
            `  Please remove the ACL files from that directory first:\n` +
            `    rm -rf "${conflict}"/acl*`,
        );
        return {
          success: false,
    ...
    // Clean up any old ACL installation first
    await this.cleanup(projectDir, options, aclDir);
    ...
    if (options.skipTarget) {
      ...
      if (this.installerConfig.commands_target_dir) {
        results.commands = await this.installCommandPointers(projectDir, aclDir, this.installerConfig, options);
      }
      return { success: true, results };
    }

    if (this.installerConfig.target_dir) {
      return this.installToTarget(projectDir, aclDir, this.installerConfig, options);
    }
   ...
   */
  async installToTarget(projectDir, aclDir, config, options) {
    ...
    results.skills = await this.installVerbatimSkills(projectDir, aclDir, targetPath, config);
    results.skillDirectories = this.skillWriteTracker.size;

    if (config.commands_target_dir) {
      results.commands = await this.installCommandPointers(projectDir, aclDir, config, options);
    }
   ...
   */
  async installCommandPointers(projectDir, aclDir, config, options = {}) {
    const result = {
      created: 0,
      updated: 0,
      skippedExisting: 0,
    ...
    for (const record of records) {
      ...
      if (filter === 'agents-only' && !(await isAgentSkill(record, aclDir))) {
   ...
   */
  async installVerbatimSkills(projectDir, aclDir, targetPath, config) {
    const aclFolderName = path.basename(aclDir);
    const aclPrefix = aclFolderName + '/';
    const csvPath = path.join(aclDir, '_config', 'skill-manifest.csv');
    ...
    for (const record of records) {
      ...
      // path is like "_acl/acl/workflows/acl-quick-flow/acl-quick-dev-new-preview/SKILL.md"
      // Strip aclFolderName prefix and join with aclDir, then get dirname
      const relativePath = record.path.startsWith(aclPrefix) ? record.path.slice(aclPrefix.length) : record.path;
      const sourceFile = path.join(aclDir, relativePath);
   ...
   * @param {string} projectDir - Project directory
   */
  async cleanup(projectDir, options = {}, aclDir = null) {
    const resolvedAclDir = aclDir || (await this._findAclDir(projectDir));
    ...
    if (options.previousSkillIds) {
      ...
      if (resolvedAclDir) {
        const removals = await this.loadRemovalLists(resolvedAclDir);
        for (const entry of removals) removalSet.add(entry);
      }
    } else if (resolvedAclDir) {
      // Uninstall flow: read from current skill-manifest.csv + removals.txt
      removalSet = await this._buildUninstallSet(resolvedAclDir);
    } else {
      removalSet = new Set();
    }

    // Strip ACL markers from copilot-instructions.md if present
    if (this.name === 'github-copilot') {
      await this.cleanupCopilotInstructions(projectDir, options);
    }

    // Strip ACL modes from .kilocodemodes if present
    if (this.name === 'kilo') {
      await this.cleanupKiloModes(projectDir, options);
    }

    // Strip ACL entries from .rovodev/prompts.yml if present
    ...
    if (this.installerConfig?.commands_target_dir) {
      ...
      const activeSkillIds = isInstallFlow ? await this._readActiveSkillIds(resolvedAclDir) : new Set();
  ...
  /**
   * Find the _acl directory in a project
   * @param {string} projectDir - Project directory
   * @returns {string|null} Path to acl dir or null
   */
  async _findAclDir(projectDir) {
    const aclDir = path.join(projectDir, ACL_FOLDER_NAME);
    return (await fs.pathExists(aclDir)) ? aclDir : null;
  }
   ...
   */
  async _buildUninstallSet(aclDir) {
    const removals = await this.loadRemovalLists(aclDir);
  ...
  /**
   * Load removal lists from all module sources in the acl directory.
   * Each module can have an optional removals.txt listing entries to remove.
   * @param {string} aclDir - ACL installation directory
   * @returns {Set<string>} Set of entries to remove
   */
  async loadRemovalLists(aclDir) {
    const removals = new Set();
    const { getProjectRoot } = require('../project-root');

    // Read project-level removals.txt (covers core and acl)
    ...
    try {
      const entries = await fs.readdir(aclDir);
      for (const entry of entries) {
        if (entry.startsWith('_')) continue;
        const removalPath = path.join(aclDir, entry, 'removals.txt');
        await this._readRemovalFile(removalPath, removals);
      }
    } catch {
      // aclDir may not exist yet on fresh install
    }
   ...
   * @param {string|null} aclDir
   * @returns {Promise<Set<string>>}
   */
  async _readActiveSkillIds(aclDir) {
    const ids = new Set();
   ...
   * When removalSet is null (legacy dirs), removes all acl-prefixed entries.
   * @param {string} projectDir - Project directory
  ...
  async cleanupTarget(projectDir, targetDir, options = {}, removalSet = new Set()) {
    ...
    for (const entry of entries) {
      ...
      // Always preserve acl-os-* utility skills regardless of cleanup mode
      if (entry.startsWith('acl-os-')) continue;

      // Surgical removal from set, or fallback to manifest+prefix detection when null
      const shouldRemove = removalSet ? removalSet.has(entry) : isAclOwnedEntry(entry, null);
  ...
  /**
   * Strip ACL-owned content from .github/copilot-instructions.md.
   * The old custom installer injected content between <!-- ACL:START --> and <!-- ACL:END --> markers.
  ...
  async cleanupCopilotInstructions(projectDir, options = {}) {
    ...
    try {
      const content = await fs.readFile(filePath, 'utf8');
      const startIdx = content.indexOf('<!-- ACL:START -->');
      const endIdx = content.indexOf('<!-- ACL:END -->');

      if (startIdx === -1 || endIdx === -1 || endIdx <= startIdx) return;

      const cleaned = content.slice(0, startIdx) + content.slice(endIdx + '<!-- ACL:END -->'.length);
      ...
      if (!options.silent) await prompts.log.message('  Cleaned ACL markers from copilot-instructions.md');
    } catch {
      if (!options.silent) await prompts.log.warn('  Warning: Could not clean ACL markers from copilot-instructions.md');
    }
  }

  /**
   * Strip ACL-owned modes from .kilocodemodes.
   * The old custom kilo.js installer added modes with slug starting with 'acl-'.
   * Parses YAML, filters out ACL modes, rewrites. Leaves file as-is on parse failure.
   */
  async cleanupKiloModes(projectDir, options = {}) {
    ...
    config.customModes = config.customModes.filter((mode) => mode && (!mode.slug || !mode.slug.startsWith('acl-')));
    ...
    if (removedCount > 0) {
      try {
        ...
        if (!options.silent) await prompts.log.message(`  Removed ${removedCount} ACL modes from .kilocodemodes`);
      } catch {
  ...
  /**
   * Strip ACL-owned entries from .rovodev/prompts.yml.
   * The old custom rovodev.js installer registered workflows in prompts.yml.
   * Parses YAML, filters out entries with name starting with 'acl-', rewrites.
  ...
  async cleanupRovoDevPrompts(projectDir, options = {}) {
    ...
    config.prompts = config.prompts.filter((entry) => entry && (!entry.name || !entry.name.startsWith('acl-')));
    ...
    if (removedCount > 0) {
      try {
        ...
        if (!options.silent) await prompts.log.message(`  Removed ${removedCount} ACL entries from prompts.yml`);
      } catch {
  ...
  /**
   * Check ancestor directories for existing ACL files in the same target_dir.
    ...
    while (current !== root && current.length > root.length) {
      ...
      try {
        if (await fs.pathExists(candidatePath)) {
          ...
          const ancestorAclDir = await this._findAclDir(current);
          const canonicalIds = await getInstalledCanonicalIds(ancestorAclDir);
          if (entries.some((e) => isAclOwnedEntry(e, canonicalIds))) {
