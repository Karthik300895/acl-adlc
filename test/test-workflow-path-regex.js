...
// Source regex (line ~1081) — uses non-capturing group for _acl
const SOURCE_REGEX = /\{project-root\}\/(?:_acl)\/([^/]+)\/workflows\/(.+)/;

// Install regex (line ~1091) — uses non-capturing group for _acl,
// consistent with source regex
const INSTALL_REGEX = /\{project-root\}\/(?:_acl)\/([^/]+)\/workflows\/(.+)/;

// ---------------------------------------------------------------------------
// Test data
// ---------------------------------------------------------------------------
const sourcePath = '{project-root}/_acl/acl/workflows/4-implementation/acl-create-story/workflow.md';
const installPath = '{project-root}/_acl/bmgd/workflows/4-production/create-story/workflow.md';
...
assert(
  sourceMatch && sourceMatch[1] === 'acl',
  'Source regex group [1] is the module name',
  `Expected "acl", got "${sourceMatch && sourceMatch[1]}"`,
);
assert(
  sourceMatch && sourceMatch[2] === '4-implementation/acl-create-story/workflow.md',
  'Source regex group [2] is the workflow sub-path',
  `Expected "4-implementation/acl-create-story/workflow.md", got "${sourceMatch && sourceMatch[2]}"`,
);
